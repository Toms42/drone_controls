function [xx,yy,zz,vxx,vyy,vzz,axx,ayy,azz,jxx,jyy,jzz] = myMinSnap(dt,waypts,v0,a0,v1,a1)
...
% maoshuyuan123@gmail.com
    ...
    % trajectory plan
    polys_x = minimum_snap_single_axis_simple(waypts(1,:),n_order,v0(1),a0(1),v1(1),a1(1));
    polys_y = minimum_snap_single_axis_simple(waypts(2,:),n_order,v0(2),a0(2),v1(2),a1(2));
    polys_z = minimum_snap_single_axis_simple(waypts(3,:),n_order,v0(3),a0(3),v1(3),a1(3));
...
function polys = minimum_snap_single_axis_simple(waypts,n_order,v0,a0,ve,ae)

syms T
ts = arrangeT(waypts,T);
%ts = sym('ts', [size(waypts,2)+1,1])
...
costfn = @(p) cost_fn(p(1:end-1), p(end), matlabFunction(Q_all));
confn = @(p) cost_fn(p(1:end-1), p(end), matlabFunction(Aeq), matlabFunction(beq));

p0 = zeros([n_coef*n_poly+1,1]);
p = fmincon(costfn, p0, [], [])
...
function [c] = cost_fn(x, T, Q_fn)
    c = x'*Q_fn(T)*x;
end

function [ceq, cieq] = nonlcon(x, T, Aeq_fn, beq_fn)
    ceq = Aeq_fn(T)*x - beq_fn;
end
